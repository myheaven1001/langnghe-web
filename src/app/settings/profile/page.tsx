...
export const metadata: Metadata = {
  title: 'Hồ sơ & xác minh — LàngNghề.vn',
};
...
export default async function BuyerProfilePage() {
  ...
  const { data: buyer } = await supabase
    .from('buyer_profiles')
    .select('id, company_name, tax_code, city, address, trust_score, verified_at, created_at')
    .eq('user_id', user.id)
    .single();

  if (!buyer) redirect('/');

  const [{ data: unreadCount }, { count: activeRfqCount }, { data: verificationsData }] =
    await Promise.all([
      supabase
        .from('notifications')
  ...
  return (
    ...
    >
      ...
      <div className="mb-[18px] flex flex-wrap items-start justify-between gap-3">
        <div>
          <div className="text-xl font-bold">Hồ sơ & xác minh</div>
          <div className="text-brand-sub mt-1 text-[12.5px]">
            Quản lý thông tin công ty và trạng thái xác minh tài khoản của bạn.
          </div>
        </div>
        {statusKey === 'approved' && (
      ...
      <div className="grid grid-cols-1 items-start gap-4 lg:grid-cols-[1fr_300px]">
        ...
        <div>
          <Card>
            <CardHeader title={<>🏢 Thông tin công ty / cửa hàng</>} />
            <CardBody padded>
              <ProfileForm
                userId={user.id}
          ...
          <Card>
            ...
            <CardBody padded>
              <div className={`mb-4 flex items-center gap-3 rounded-lg border p-3.5 ${toneClasses.block}`}>
                ...
                <div>
                  <div className={`text-[13.5px] font-bold ${toneClasses.title}`}>
                    {statusInfo.title}
                  </div>
                  <div className="text-brand-sub mt-0.5 text-[11.5px] leading-relaxed">
                    {statusKey === 'none' &&
                      'Tải lên giấy phép kinh doanh để xác minh tài khoản và được xưởng ưu tiên phản hồi.'}
                    {statusKey === 'pending' &&
                      `Giấy tờ đã gửi lúc ${formatVnDate(latest!.created_at)}. Đội kiểm duyệt sẽ xem xét trong 1–2 ngày làm việc.`}
                    {statusKey === 'approved' &&
                      `Hồ sơ doanh nghiệp của bạn đã được duyệt${latest?.verified_at ? ` vào ${formatVnDate(latest.verified_at)}` : ''}. Tài khoản được ưu tiên hiển thị khi gửi RFQ.`}
                    {statusKey === 'rejected' &&
              ...
              {statusKey === 'approved' && (
                <div className="text-brand-light mb-3 text-[11px]">
                  Giấy tờ đã duyệt và không thể chỉnh sửa trực tiếp. Nếu thông tin doanh nghiệp thay
                  đổi, vui lòng tải lên giấy tờ mới để xác minh lại.
                </div>
              )}

              <VerificationUpload buyerId={buyer.id} nextAttemptNumber={nextAttemptNumber} />
            </CardBody>
          </Card>
        ...
        <div>
          <div className="border-brand-border mb-4 rounded-[10px] border bg-white p-4">
            <div className="mb-3.5 border-b border-[#F2F0EC] pb-3.5 text-center">
              ...
              <div className="flex items-center justify-center gap-3">
                ...
                <div className="text-brand-sub text-left text-[11px] leading-relaxed">
                  Điểm uy tín
                  <br />
                  <strong className="text-brand-ink">{trustLabel(buyer.trust_score)}</strong>
                </div>
              </div>
            </div>
            ...
            {buyer.verified_at && (
              <div className="flex justify-between border-b border-[#F2F0EC] py-[7px] text-xs last:border-b-0">
                ...
                <span className="text-brand-ink text-right font-semibold">
                  {formatVnDate(buyer.verified_at)}
                </span>
              </div>
            )}
            <div className="flex justify-between py-[7px] text-xs">
              <span className="text-brand-sub">Loại tài khoản</span>
              <span className="text-brand-ink text-right font-semibold">Buyer</span>
            </div>
          </div>
          ...
          <div className="border-brand-border mb-4 rounded-[10px] border bg-white p-4">
            ...
            {[
              ['⚡', 'Xưởng ưu tiên phản hồi báo giá cho tài khoản đã xác minh'],
              ['📈', 'Tăng điểm uy tín, mở khóa hạn mức RFQ cao hơn'],
              ['🔒', 'Bảo vệ giao dịch — đủ điều kiện dùng thanh toán Escrow'],
              ['🛡️', 'Huy hiệu "Đã xác minh" hiển thị công khai khi gửi RFQ'],
            ].map(([icon, text]) => (
